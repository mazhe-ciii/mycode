...
if __name__ == '__main__':
    ...
    cfg_path = os.path.join(file_path, 'config', 'config_intf_xdr.ini')
    cfg.read(cfg_path)
    ...
    # local_path = '/data/intf_xdr_backup/dr_ps/20180703'
    # remote_path = '/data/intf_xdr_backup/dr_ps/20180703'
    log_name = '%s_%s_%s.log' % (log_prefix, deal_type, datetime.now().strftime('%Y%m%d'))
    # log_name = '%s_%s_20180703.log' % (log_prefix, deal_type)
    # 设置日志文件输出
    ...
    # 查找本地待上传目录,由bak_disk配置项中配置的备份盘搜索
    bak_disk = cfg.get(deal_type, 'bak_disk')
    for item in bak_disk.split(','):
        check_path = os.path.join(item, local_path)
        if os.path.exists(check_path):
            local_path = check_path
            logger.info('Find local path ' + local_path)
            break
